...
GhostFix AI is a local-first debugging assistant. It runs or watches terminal commands, detects tracebacks and runtime errors, extracts nearby source context, explains the likely root cause, recommends a fix, and applies only small validated Python patches when the safety policy allows it.
